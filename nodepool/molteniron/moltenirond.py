...
class DataBase():
    ...
    def create_metadata(self):
        ...
        if DEBUG: print "create_metadata: Calling metadata.create_all"
        metadata.create_all(self.engine, checkfirst=True)
        if DEBUG: print "create_metadata: Finished"
    ...
    def deallocateOwner(self, owner_name):
        ...
        try:
            with self.session_scope() as session:
                nodes = session.query(Nodes.id).filter_by(provisioned=owner_name)
                for node in nodes:
    ...
    def get_field(self, owner_name, field):
        ...
        try:
            with self.session_scope() as session:
                ...
                if nodes.count() == 0:
                    return {'status': 404,
                            'message': '%s does not own any nodes' % owner_name}

...
#   mysql_password      => hiera('ciwatch::mysql_password', 'XXX'),
#   mysql_root_password => hiera('ciwatch::mysql_root_password', 'XXX'),
# }
#
# === Authors
#
...
class ciwatch (
  ...
  exec { 'install_ciwatch' :
    provider    => 'shell',
    command     => 'pip install -e /opt/ciwatch',
  ...
  ciwatch::initd_service { 'ciwatch-ui':
    exec_cmd          => 'ci-watch-server',
    short_description => 'CI Watch Web UI',
    runas_user        => 'ciwatch',
  }

  ciwatch::initd_service { 'ciwatch-gerrit-listener':

...
class Nodes(declarative_base()):
    ...
    ram_mb        = Column('ram_mb', Integer) # @TODO - why not just ram?
    disk_gb       = Column('disk_gb', Integer) # @TODO - why not just disk?
...
class DataBase():
    ...
    def __init__(self,
        ...
        if self.db_type == TYPE_MYSQL:
            ...
            self.engine = create_engine("sqlite://%s:%s@%s/%s"
                                        % (self.user,
                                           self.passwd,
                                           self.host,
                                           self.database, ),
    ...
    def close(self):
        if DEBUG: print "close: Calling engine.dispose()"
        self.engine.dispose ()
        if DEBUG: print "close: Finished"
    ...
    def session_scope(self):
        ...
        try:
            ...
            if DEBUG:
                print "Exception caught in session_scope: %s %s" % (e,traceback.format_exc(4))
            session.rollback()
            raise
        finally:
            session.close()
    ...
    def create_db(self):
        ...
        if DEBUG: print "create_db: Calling metadata.create_all"
        metadata.create_all(self.engine, checkfirst=True)
        if DEBUG: print "create_db: Finished"
    ...
    def deallocateBM(self, id):
        ...
        try:
            with self.session_scope() as session, \
                ...
                stmt = stmt.values(status="ready", provisioned="", timestamp=None)
        ...
        try:
            ...
            with self.session_scope() as session, \
                ...
                if node.has_key('timestamp'):
                    ...
                    if DEBUG: print ("timestamp_str = %s" % (timestamp_str, ))
                    if len(timestamp_str) != 0 and timestamp_str != "-1":
                        ts = self.to_timestamp(time.gmtime(float(timestamp_str)))
                        if DEBUG: print ("ts = %s" % (ts, ))
                        stmt = stmt.values(timestamp=ts)
                if DEBUG:
            ...
            with self.session_scope() as session:
                ...
                for node in nodes:
                    ...
                    if DEBUG:
                        ...
                        print ("node.timestamp      = %s" % (node.timestamp, ))
                        print ("elapsedTime         = %s" % (elapsedTime, ))
                        print ("elapsedTime.seconds = %s" % (elapsedTime.seconds, ))
    ...
    def doClean(self, node_id):
        ...
        try:
            with self.session_scope() as session, \
                ...
                logstring = "The node at %s has been cleaned." % (node.ipmi_ip,)
                log(self.conf, logstring)
    ...
    def get_field(self, ownerName, field):
        ...
        try:
            with self.session_scope() as session:
                ...
                if nodes.count() == 0:
                    return {'status': 404,
                            'message': '%s does not own any nodes' % ownerName}

...
function invoke-fc-passthrough {
    ...
    #
    # example: invoke-fc-passthrough cinder-init2 root /opt/nodepool-scripts/passthrough /root/keystonerc_jenkins
    ...
    # Get our NOVA_ID
    NOVA_LIST=$(ssh -i $PROVIDER_KEY $PROVIDER_USER@$PROVIDER "source $JENKINS_OPENRC && nova list")
    local nova_result=$?
    ...
    NOVA_DETAILS=$(ssh -i $PROVIDER_KEY $PROVIDER_USER@$PROVIDER "source $JENKINS_OPENRC && nova show $NOVA_ID")
    nova_results=$?
    ...
    for pci in $fc_pci_device; do
        ...
        echo $XML
        local fcoe=`mktemp --suffix=_fcoe.xml`
        echo $XML > $fcoe
...
function install-fc-driver {
    ...
    local device_path=$(sudo systool -c fc_host -v | grep "Device path")
    if [[ ${#device_path}  -eq 0 ]]; then
        echo "FC Attach failed. Aborting."
        exit 1
    fi
}
